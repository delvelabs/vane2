...
class TestActivePluginFinder(TestCase):
    ...
    def test_load_plugins_files_signatures(self):
        filename = join(dirname(__file__), "samples/plugins_files.json")

        self.plugin_finder.load_plugins_files_signatures(filename)
    ...
    def test_enumerate_plugins_fetch_all_version_definitions_files_for_each_plugin(self):
        ...
        with loop_context() as loop:
            plugins = loop.run_until_complete(self.plugin_finder.enumerate_plugins(target))
    ...
    def test_enumerate_plugins_skip_plugins_with_no_files(self):
        ...
        self.plugin_finder.plugins_file_list = FileListGroup(key="plugins", producer="unittest", file_lists=[plugin2])
        self.plugin_finder.version_identification.fetch_files = make_mocked_coro(["fetched files"])
        target = "http://www.example.com"

        with loop_context() as loop:
            loop.run_until_complete(self.plugin_finder.enumerate_plugins(target))
