...
class TestActivePluginFinder(TestCase):
    ...
    def setUp(self):
        ...
        self.plugins_finder = ActivePluginsFinder(MagicMock(), self.target_url)
        self.plugins_finder.file_fetcher = MagicMock()

    def test_load_all_plugins_files_signatures(self):
        file_path = join(dirname(__file__), "samples")

        self.plugins_finder.load_plugins_files_signatures(file_path, False, False)

        self.assertEqual(self.plugins_finder.plugins_file_list_group, self.plugin_list)
    ...
    def test_load_popular_plugins_files_signatures(self):
        ...
        self.plugins_finder.load_plugins_files_signatures(file_path, True, False)

        self.assertEqual(self.plugins_finder.plugins_file_list_group, popular_plugin_list)
    ...
    def test_load_vulnerable_plugins_files_signatures(self):
        ...
        self.plugins_finder.load_plugins_files_signatures(file_path, False, True)

        self.assertEqual(self.plugins_finder.plugins_file_list_group, vulnerable_plugin_list)

    def test_load_vulnerable_and_popular_plugins_merge_file_lists(self):
        ...
        self.plugins_finder.load_plugins_files_signatures(file_path, True, True)

        self.assertIn(popular_plugin_list.file_lists[0], self.plugins_finder.plugins_file_list_group.file_lists)
        self.assertIn(vulnerable_plugin_list.file_lists[0], self.plugins_finder.plugins_file_list_group.file_lists)
    ...
    def test_enumerate_plugins_fetch_version_definitions_files_for_plugin(self):
        with loop_context() as loop:
            ...
            self.plugins_finder.file_fetcher.request_files.return_value = loop.create_task(request_files())
            self.plugins_finder.plugins_file_list_group = FileListGroup(key="plugins", producer="",
                                                                        file_lists=[self.plugin0_file_list])

            plugins, errors = loop.run_until_complete(self.plugins_finder.enumerate_plugins())

            self.plugins_finder.file_fetcher.request_files.assert_has_calls(
    ...
    def test_enumerate_plugins_skip_plugins_with_no_files(self):
        with loop_context() as loop:
            ...
            self.plugins_finder.plugins_file_list_group = FileListGroup(key="plugins", producer="", file_lists=[plugin0])

            plugins, errors = loop.run_until_complete(self.plugins_finder.enumerate_plugins())

            self.plugins_finder.file_fetcher.request_files.assert_not_called()
    ...
    def test_enumerate_plugins_return_list_of_dict_with_plugin_key_and_fetched_files(self):
        ...
        with loop_context() as loop:
            ...
            plugins_finder = ActivePluginsFinder(hammertime, self.target_url)
            plugins_finder.plugins_file_list_group = self.plugin_list

            plugins, errors = loop.run_until_complete(plugins_finder.enumerate_plugins())
